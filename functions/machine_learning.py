...
def model_LR(facies, features):
    ...
    x_train, x_test, y_train, y_test = train_test_split(
        features, facies, train_size=0.8, random_state=123
    )
...
def predict_2d_RelAI_Seis_SeisInv(df_facies, relai, seis, seis_inv, model):
    ...
    for every_col in range(len(df_facies.columns)):
        ...
        map_facies_1[non_empty_facies.index, every_col] = map_facies[
            non_empty_facies.index, every_col
        ]
        df_f_comparison[new_df_1.index, every_col] = df_facies.iloc[
            new_df_1.index, every_col
        ]
...
def predict_2d_Seis_SeisInv(df_facies, seis, seis_inv, model):
    ...
    for every_col in range(len(df_facies.columns)):
        # concat every column
        new_df = pd.concat(
            [seis.iloc[:, every_col], seis_inv.iloc[:, every_col]], axis=1
        )
...
###########################################################################################################
def predict_2d_RelAI_Seis_Envel_InstFreq_Depth(
    df_facies, relai, seis, envel, inst_freq, depth, model
):
    # create empty Numpy array with the same shape as facies
    r, c = np.shape(df_facies)
    map_facies = np.zeros((r, c))
...
################################################################################################################
def predict_2d_RelAI_Seis_Envel_InstFreq(
    df_facies, relai, seis, envel, inst_freq, model
):
    # create empty Numpy array with the same shape as facies
    r, c = np.shape(df_facies)
    map_facies = np.zeros((r, c))
...
def feature_importance_plot(model, x_train, y_train, random_state):
    ...
    res = permutation_importance(
        model, x_train, y_train, scoring="accuracy", random_state=random_state
    )
    importance = res.importances_mean
    importance
    importance_res = pd.Series(importance, index=x_train.columns).sort_values(
        ascending=True
    )
    importance_res
    # Plot the results
...
def confusion_matrix_prediction(
    df_facies_comparison, facies_pred, col_number, facies_class
):
    from sklearn.metrics import f1_score
    from sklearn.metrics import precision_recall_fscore_support
    from sklearn import metrics
    ...
    df_f_comparison_col = df_f_comparison_pd.drop(
        df_f_comparison_pd.columns[col_number], axis=1
    )
    ...
    display_conf_matrix = ConfusionMatrixDisplay(
        confusion_matrix=conf_matrix, display_labels=np.array(facies_class)
    )
...
###########################################################################################################
def predict_2d_RelAI_Seis_Envel_InstFreq_SeisInv(
    df_facies, relai, seis, envel, inst_freq, seis_inv, model
):
    # create empty Numpy array with the same shape as facies
    r, c = np.shape(df_facies)
    map_facies = np.zeros((r, c))
...
########################################################################################################################
def predict_facies_3D(
    facies, relai_std, seis_std, envel_std, inst_freq_std, seis_inv_std, model
):
    ...
    for every_x in range(facies.shape[0]):
        for every_y in range(facies.shape[1]):
            ...
            map_facies[every_x, every_y, new_features.index] = model.predict(
                new_features
            )

            # Filter
            map_facies_1[every_x, every_y, non_empty_facies.index] = map_facies[
                every_x, every_y, non_empty_facies.index
            ]
            df_f_comparison[every_x, every_y, new_features.index] = facies[
                every_x, every_y, new_features.index
            ]
