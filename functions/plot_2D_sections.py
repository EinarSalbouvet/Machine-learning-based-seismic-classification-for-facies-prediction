...
def plot_2D_section(
    data_file, extent_plot, color_map="seismic", number_of_facies=5, list_of_wells=None
):
...
def difference_map(df_facies_comparison, facies_predicted, extent, list_of_wells=None):
    ...
    df1 = df1.apply(np.sign).replace(
        {-4: 1, -3: 1, -2: 1, -1: 1, 0: 0, 1: 1, 2: 1, 3: 1, 4: 1}
    )
    facies_difference_result = np.array(df1)

    cmap = matplotlib.colors.ListedColormap(["green", "red"])
    im = ax.imshow(
        facies_difference_result,
        cmap=cmap,
        vmin=0,
        vmax=1,
        aspect="auto",
